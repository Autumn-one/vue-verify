...
export default {
  verify: {
    inserted: function (el, binding, vnode) {
      ...
      if (!vm.$verify) {
        vm.$verify = {
          doVerify (namespace = 'default') { // 手动验证的方法
            // 默认是 default
            vm.$verify[namespace + 'El'].forEach(el => {
              // 脚本触发事件
      ...
      // 判断数组中是否有 equal 修饰符，用于判断输入框内容是否相等 如果有则从数组中删除并且将 verifyEqual 设置为true 没有则只将其设置 false
      let equalIndex = regExpArr.indexOf('equal')
      let verifyEqual = equalIndex === -1 ? false : (regExpArr.splice(equalIndex, 1), true)
      ...
      function verifyInput (eventType) {
        ...
        // 输入框空验证
        let emptyVerifyFlag = inputValue.trim() !== ''
        // 验证规则，获取最终正则验证的结果，此时还没有检验 equal 验证
        let regVerifyFlag = regExpArr.every(key => regExpConfig[key].test(inputValue))
        ...
        if (el.value === '' || ((regVerifyFlag && equalVerifyFlag) && eventType === 'blur')) {
          el.style.border = originBorderStyle
        } else if (regVerifyFlag && equalVerifyFlag) {
          // el.style.border = '3px solid green'
          el.style.border = originBorderStyle // 这句是 ui 要的效果，不是我本来要实现的效果
        } else if ((!_data) && (!regVerifyFlag)) {
